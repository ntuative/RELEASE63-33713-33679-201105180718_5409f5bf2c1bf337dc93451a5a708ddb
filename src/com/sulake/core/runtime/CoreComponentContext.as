package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1493:String = "asset-libraries";
      
      private static const const_1490:String = "library";
      
      private static const const_1495:String = "service-libraries";
      
      private static const const_1494:String = "library";
      
      private static const const_1488:String = "component-libraries";
      
      private static const const_1491:String = "library";
      
      private static const const_1496:String = "error_data";
      
      private static const const_1489:String = "error_cat";
      
      private static const const_1492:String = "error_desc";
       
      
      private var var_376:LibraryLoaderQueue;
      
      private var var_816:IEventDispatcher;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         {
            this.var_266.push(new Array());
            this.var_661.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_406.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            ...
            case Core.const_790:
               ...
               var_406.addChild(new ProfilerViewer(this.var_212));
               break;
            case Core.const_620:
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_266[i] as Array;
            ...
            if(var_406)
            {
               var_406.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_406 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1496,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1489,String(param3));
         ErrorReportStorage.setParameter(const_1492,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_816 = loadingEventDelegate;
         if(this.var_376 == null)
         {
         ...
         try
         {
            node = config.child(const_1493)[0];
            if(node != null)
            {
               list = node.child(const_1490);
         ...
         try
         {
            node = config.child(const_1495)[0];
            if(node != null)
            {
               list = node.child(const_1494);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1488)[0];
            if(node != null)
            {
               list = node.child(const_1491);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1867);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_816)
         {
            this.var_816.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_816)
         {
            ...
            this.var_816.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_816)
                  {
                     this.var_816.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_542 - this.var_376.length,this.var_542,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_212 ? 0 : this.var_1528 & 0;
         if(_loc3_ == Core.const_620)
         {
            this.var_266[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_266[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_212 ? 0 : this.var_1528 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_104)
         {
            _loc3_ = this.var_266[_loc5_] as Array;
            if(_loc4_ == Core.const_620)
            {
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_661[priority] = 0;
            receivers = this.var_266[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_406.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_104)
         {
            t = getTimer();
            ...
            {
               if(this.var_661[priority] < priority)
               {
                  ++this.var_661[priority];
                  skip = true;
               }
            }
            if(!skip)
