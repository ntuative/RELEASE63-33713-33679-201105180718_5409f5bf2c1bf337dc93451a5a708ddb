package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_545 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_545 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2286 = _loc3_.localizationId;
         this.var_347 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_545,this);
               if(_loc5_)
               {
                  this.var_347 = _loc5_.colors;
         ...
         this._offers.add(this.var_545,_loc3_);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_347)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_499 < 0 || this.var_499 > this.var_347.length)
         {
            this.var_499 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2286)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_545 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_202.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_545,_loc5_),this.getRaceLocalizationKey(this.var_545,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_202 == null || this.var_721 >= this.var_202.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_499 >= 0 && this.var_499 < this.var_347.length)
         {
            _loc7_ = this.var_347[this.var_499] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_202[this.var_721] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_545,_loc3_,_loc2_,AvatarScaleType.const_58,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_317,AvatarAction.const_967);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_499 >= this.var_347.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_347[this.var_499] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_202[this.var_721] as int;
