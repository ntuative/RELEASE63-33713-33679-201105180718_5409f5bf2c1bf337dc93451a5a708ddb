package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_434:IFriendCategoriesDeps;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            status = 1;
            this.updateCategories(p.method_12);
            status = 2;
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_434.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_434.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_434.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_434.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_434.view.setNewMessageArrived();
                  this.var_434.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_1317).received = true;
