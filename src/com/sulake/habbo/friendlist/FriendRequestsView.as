package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      {
         this.var_2053 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         this.var_2054 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         this.var_2054.procedure = this.onDismissAllButtonClick;
         this.var_2053.procedure = this.onAcceptAllButtonClick;
         this.refreshButtons();
      }
      ...
      public function refreshRequestEntry(param1:FriendRequest) : void
      {
         ...
         else if(param1.state == FriendRequest.const_1189)
         {
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         if(this._friendList && this._friendList.events)
         {
            this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_316,param1));
         }
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_437 && _loc4_.state != FriendRequest.const_378)
            {
               _loc4_.state = FriendRequest.const_378;
               this.refreshRequestEntry(_loc4_);
               if(this._friendList && this._friendList.events)
               {
                  this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_316,_loc4_.requestId));
               }
            }
         }
         this.refresh();
      ...
      private function refreshButtons() : void
      {
         ...
         this.setEnabled(this.var_2053,_loc1_);
         this.setEnabled(this.var_2054,_loc1_);
      }
