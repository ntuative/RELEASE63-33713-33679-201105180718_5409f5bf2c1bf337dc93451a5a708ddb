package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_731:IFigurePartSet;
      ...
      private const const_2205:int = 13421772;
      
      private const const_2204:int = 16777215;
      
      private const const_364:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1726 = Math.max(this.var_1726,_loc6_.colorLayerIndex);
            }
         }
         this.var_422 = this.var_68.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_702 != null && !this.var_1727)
            {
               _loc3_ = this.var_702;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_731)
            {
               switch(this.var_731.clubLevel)
               {
                  case HabboClubLevelEnum.const_57:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_909;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_944;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_364[_loc6_] + "_" + FigureData.const_157;
               _loc4_ = this.var_422.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_364.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_364[_loc6_] + "_" + FigureData.const_157;
                  _loc4_ = this.var_422.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_598 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1728)
               {
                  ...
                  this.var_1728 = _loc9_.content as BitmapData;
               }
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1727 && _loc2_.colorLayerIndex > 0)
               {
