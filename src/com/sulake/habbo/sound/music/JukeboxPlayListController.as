package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_723 = [];
         this.var_723.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_723.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_723.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         if(param1 < 0 || param1 >= this.var_502.length)
         {
            return null;
         }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_692 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            ...
            this.var_474 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         this.var_1712 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_101,HabboMusicPrioritiesEnum.const_132,_loc3_.currentSongId,_loc3_.currentPosition));
      }
         ...
         while(_loc2_ < this.length)
         {
            ...
            if(_loc4_.id == param1.id)
            {
               ...
               if(_loc6_ != null)
               {
                  ...
                  this.var_502[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_724.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_724.splice(_loc3_,1);
         }
         if(this.var_724.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_103));
